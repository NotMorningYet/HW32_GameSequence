...
namespace Assets._Project.Develop.Runtime.Utilities.DataManagement.DataProviders
{
    ...
    {
        private readonly List<IDataWritable<TData>> _writers = new();
        private readonly List<IDataReadable<TData>> _readers = new();
        ...
        public void RegisterWriter(IDataWritable<TData> writer)
        {
        ...
        public void RegisterReader(IDataReadable<TData> reader)
        {
        ...
        {
            yield return _saveLoadService.Exists<TData>(result => onExistsResult(result));
        }
        ...
        {
            foreach (IDataReadable<TData> reader in _readers)
                reader.ReadFrom(_data);
        }

        private void UpdateDataFromWriters()
        {
            foreach (IDataWritable<TData> writer in _writers)
